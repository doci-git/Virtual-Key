(() => {
  ...
  let MAX_CLICKS = Number.POSITIVE_INFINITY; // no click limit
  let TIME_LIMIT_MINUTES = Number.POSITIVE_INFINITY; // manual session time limit disabled
  ...
  // Orari di check‑in
  let CHECKIN_START_TIME = "00:00";
  let CHECKIN_END_TIME = "23:59";
  let CHECKIN_TIME_ENABLED = false;
  ...
  function applyFirebaseSettings(settings) {
    ...
    // Ignore manual session time-limit settings
    // Check-in time-of-day removed — ignore time settings from Firebase
  ...
  // TEMPO/SESSIONE
  // =============================================
  async function setUsageStartTime() { /* manual session limit removed */ }
  ...
  function updateStatusBar() {
    ...
    if (mainDoorCounter) mainDoorCounter.textContent = `Unlimited`;
    if (aptDoorCounter) aptDoorCounter.textContent = `Unlimited`;

    // no timeRemaining countdown
  }
  ...
  function setClicksLeft(key, count) {
    // no-op: unlimited clicks
  }

  function updateButtonState(device) {
    const btn = qs(device.button_id);
    if (!btn) return;
    btn.disabled = !isCheckinTime();
    if (!isCheckinTime()) {
  ...
  function checkExpiredLinks() {
    ...
    Object.keys(secureLinks).forEach((linkId) => {
      ...
      if (endTs < Date.now() && link.status === "active") {
  ...
  async function handleSecureToken() {
    ...
    try {
      ...
      showTokenNotification(null, !!currentTokenCustomCode);
      maybeCleanUrl();
    startTokenExpirationCheck(linkData.validTo || linkData.expiration);
  ...
  function startTokenRealtimeListener(token) {
    ...
    tokenRef.on("value", (snap) => {
      ...
      const d = snap.val();
      const now = Date.now();
      const exhausted = false; // usage limit disabled for tokens
      const endTs = (d && (d.validTo || d.expiration)) || 0;
  ...
  function validateSecureToken(linkData) {
    try {
      ...
      if (linkData.validFrom && Date.now() < linkData.validFrom)
        return { valid: false, reason: "Token non ancora attivo" };
      const endAt = linkData.validTo || linkData.expiration;
      if (endAt < Date.now())
  ...
  async function incrementTokenUsage(token, linkData) { /* usage limit removed */ }
  ...
  function showTokenNotification(remainingUses, hasCustomCode) {
    ...
    n.innerHTML = `
      ...
      <div>
        ...
        <div style="font-size:12px;opacity:.9;">Valido solo nelle date impostate</div>
        ${custom}
